= image:images/tpo42-logo.png[tpo42] Framework
...
# copy the quality gates
cp -r tpo42-templates/{lefthook.yml,docker-compose.yml,container,.editorconfig,.hadolint.yaml,.yamllint,.gitlint} \
  your-shiny-project/
...
# same for the quality gates — gitleaks needs no baseline to maintain
lefthook install
git add lefthook.yml docker-compose.yml container \
  .editorconfig .hadolint.yaml .yamllint .gitlint
git commit -s -m "docs: introduce quality gates ..."
----

The skills under `.claude/skills/` are left out of the copy above on purpose.
Copying them makes a second source that ages quietly; https://github.com/Getty/manage-skills[manage-skills] hardlinks them from one place instead, so an improvement reaches every project that linked it and your teammates still get a normal file when they clone:

[source,bash]
----
manage-skills sources add /path/to/tpo42-templates/.claude/skills
manage-skills link xref
----

Copying works too, if you would rather not add a tool for it — the skills are plain Markdown and
carry no dependencies.
Which way you go stays your decision; the templates only provide the source.

==== Local lefthook Setup

`lefthook.yml` names tools, never containers.
Where those tools come from is your decision and not the repository's — declare it once in `lefthook-local.yml`, which is git-ignored.
...
Then pick a source for the tools.
Using the containers shipped here:

A container does not see your `~/.gitconfig`, so tools that ask git who you are — `gitlint`, for one — find nothing.
Two values in a git-ignored `.env` fix that for every containerised tool at once, and the same mechanism works unchanged on a CI runner:

[source,bash]
----
printf 'GIT_USER_NAME=%s\nGIT_USER_EMAIL=%s\n' \
  "$(git config user.name)" "$(git config user.email)" > .env

docker compose up -d

cat > lefthook-local.yml <<'EOF'
templates:
  adoc: docker compose exec -T tpo42-tpl-adoc
  alint: docker compose exec -T tpo42-tpl-alint
  dtc: docker compose exec -T tpo42-tpl-dtc
  mini: docker compose exec -T tpo42-tpl-mini
EOF
----

`docker compose exec` against warm services costs one container start per session.
`docker compose run --rm -T` works as well and manages no lifecycle, at the price of one start per job — noticeable with eight jobs on a pre-commit hook.

Podman, another compose implementation, or an entirely different source only change the prefixes above.
Nothing else in this repository assumes a runtime.

Having no `lefthook-local.yml` at all is equally valid: the templates stay empty and the tools are expected on `PATH`.
That is how GitHub Actions runs these checks, a runner being a VM rather than a container.

If a GUI git client runs your hooks, it will not see your shell profile.
An rc file gives those hooks a PATH and can be shared across projects:
